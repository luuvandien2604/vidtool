"""Master Cut v3 - Cinematic Storytelling Motion Engine with Lifecycle Exits and Seamless Subtitles.

Features:
1. Dynamic Entrance & Exit Lifecycle:
   - Map Card exits gracefully at t=9.0s to prevent screen clutter.
   - Clean spacing allows the 1961 authentic archival photo to breathe.
2. Realistic Cartographic Vector Germany Map:
   - Accurate border paths for West Germany (FRG) and East Germany (GDR) + Baltic coast.
   - Animated migration route arrow from Berlin to West.
3. 100% Seamless Dynamic Subtitles (0.0s - 24.5s):
   - Unbroken subtitle experience at bottom center.
   - Micro-timed active word highlight in radiant gold (#FFD700).
   - Dynamic glow aura on climax keywords ('nỗi sợ hãi', 'sự chia rẽ').
4. Cinematic Color Harmony & Optical Grain:
   - Harmonized lighting across all collage elements.
   - 30fps dynamic grain and vintage lens vignette.
"""
...
SUBTITLE_CHUNKS = [
    ...
    (4.30, 8.60, WORD_TIMINGS[15:31]),
    (8.70, 13.40, WORD_TIMINGS[31:44]),
    (13.45, 17.10, WORD_TIMINGS[44:57]),
    (17.20, 24.50, WORD_TIMINGS[57:79]),
]


def build_cinematic_layers(work_dir: Path) -> dict[str, Path]:
    ...
    defs_xml, paper_xml = render_paper_panel_svg(
        width=740.0, height=1080, fill_color="#131417", seed=42
    )
    ...
    # 5. Editorial Kinetic Callout Block 1
    ...
    # 6. Editorial Kinetic Callout Block 2 (t: 9.5s)
    ...
    # 8. Realistic Geographic Vector Cartography Card (Top-Right)
    map_svg = """<svg xmlns="http://www.w3.org/2000/svg" width="1920" height="1080" viewBox="0 0 1920 1080">
      <defs>
        <filter id="mapShadow"><feDropShadow dx="8" dy="14" stdDeviation="14" flood-color="#000000" flood-opacity="0.8"/></filter>
      ...
      <g transform="translate(1420, 60) rotate(-2)" filter="url(#mapShadow)">
        ...
        <!-- West Germany realistic geographic outline -->
        <path d="M 60,75 C 90,65 140,55 180,60 C 200,62 205,75 210,95 L 210,180 L 195,225 L 205,275 L 180,310 L 120,315 L 65,295 L 45,250 L 50,175 L 40,125 Z" fill="#24384A" stroke="#466A8A" stroke-width="2" />
        <text x="125" y="175" text-anchor="middle" font-family="'DejaVu Sans', sans-serif" font-size="18" font-weight="900" fill="#A8D0F0" letter-spacing="1">TÂY ĐỨC</text>
        <text x="125" y="200" text-anchor="middle" font-family="'DejaVu Sans', sans-serif" font-size="12" font-weight="700" fill="#6B95B8">(Tư Bản)</text>

        <!-- East Germany realistic geographic outline -->
        <path d="M 210,95 C 235,75 285,60 330,65 C 375,70 395,95 400,125 L 395,185 L 375,245 L 335,285 L 275,305 L 205,275 L 195,225 L 210,180 Z" fill="#6E231D" stroke="#A83C32" stroke-width="2" />
        <text x="310" y="175" text-anchor="middle" font-family="'DejaVu Sans', sans-serif" font-size="18" font-weight="900" fill="#F5A89E" letter-spacing="1">ĐÔNG ĐỨC</text>
        <text x="310" y="200" text-anchor="middle" font-family="'DejaVu Sans', sans-serif" font-size="12" font-weight="700" fill="#BD6258">(Cộng Sản)</text>

        <!-- Division Border line -->
        <line x1="208" y1="62" x2="202" y2="280" stroke="#E1B400" stroke-width="3" stroke-dasharray="6,4" />

        <!-- Berlin Enclave Marker -->
        <circle cx="285" cy="120" r="9" fill="#E1B400" stroke="#FFFFFF" stroke-width="2" />
        <circle cx="285" cy="120" r="16" fill="none" stroke="#E1B400" stroke-width="1.5" stroke-opacity="0.8" stroke-dasharray="3,3" />
        <text x="302" y="125" font-family="'DejaVu Sans', sans-serif" font-size="14" font-weight="900" fill="#FFFFFF">BERLIN</text>

        <!-- Dotted escape arrow -->
        <path d="M 275 130 Q 230 155 168 145" fill="none" stroke="#FFFFFF" stroke-width="3" stroke-dasharray="5,4" />
        <polygon points="163,145 174,138 174,152" fill="#FFFFFF" />
    ...
    sign_svg = """<svg xmlns="http://www.w3.org/2000/svg" width="1920" height="1080" viewBox="0 0 1920 1080">
      ...
      <g transform="translate(1480, 710) rotate(2)" filter="url(#signShadow)">
        <rect x="0" y="0" width="370" height="230" rx="6" fill="#EAE5D8" stroke="#524E48" stroke-width="4" />
        <rect x="8" y="8" width="354" height="214" rx="4" fill="none" stroke="#8C8476" stroke-width="1.5" />
        
        <circle cx="16" cy="16" r="4" fill="#403C36" />
        <circle cx="354" cy="16" r="4" fill="#403C36" />
        <circle cx="16" cy="214" r="4" fill="#403C36" />
        <circle cx="354" cy="214" r="4" fill="#403C36" />

        <text x="185" y="55" text-anchor="middle" font-family="'DejaVu Sans', sans-serif" font-size="34" font-weight="900" fill="#B81D13" letter-spacing="1.5">Achtung !</text>
        <text x="185" y="102" text-anchor="middle" font-family="'DejaVu Sans', sans-serif" font-size="20" font-weight="700" fill="#161618">Sie verlassen jetzt</text>
        <text x="185" y="145" text-anchor="middle" font-family="'DejaVu Sans', sans-serif" font-size="26" font-weight="900" fill="#161618" letter-spacing="1">West-Berlin</text>
        <text x="185" y="185" text-anchor="middle" font-family="'DejaVu Sans', sans-serif" font-size="12" font-weight="600" fill="#5E5950">Zone Border Checkpoint / 1961</text>

        <polygon points="12,-10 60,-16 52,18 4,24" fill="#F4F0DC" fill-opacity="0.8" />
        <polygon points="310,-14 360,-8 352,24 302,18" fill="#F4F0DC" fill-opacity="0.8" />
      </g>
    </svg>"""
...
def draw_dynamic_subtitles_v3(
    frame: Image.Image,
    current_time: float,
    font: ImageFont.FreeTypeFont,
) -> None:
    """Render continuous, unbroken active-word highlighted subtitle pill across all 24.5s."""
    active_chunk = None
    ...
    for w_start, w_end, w_text in words_data:
        ...
        # Highlight golden keywords during climax
        is_climax_word = ("nỗi" in w_text or "sợ" in w_text or "hãi" in w_text or "chia" in w_text or "rẽ" in w_text)
        
        if is_active:
            fill_color = (255, 215, 0, 255) # Bright Gold
            draw.text((curr_x + 1, sub_y + 1), w_str, font=font, fill=(0, 0, 0, 220))
        elif is_climax_word and current_time >= 21.0:
            fill_color = (235, 190, 0, 255) # Stays gold after spoken
        else:
...
def render_master_cut_v3_frames(
    ...
    """Render master cut frames with dynamic entrance & exit lifecycles."""
    ...
    print(f"🎬 Rendering {total_frames} Master Cut v3 frames ({duration_sec:.1f}s @ {fps}fps)...")
    ...
    for f_idx in range(total_frames):
        ...
        # -------------------------------------------------------------
        # 1. 1961 Archival Hero Photo: Cinematic Push-in with climax deepening
        # -------------------------------------------------------------
        if t < 17.0:
            kb_scale = 1.00 + 0.08 * (t / 17.0)
        else:
            # Deepen zoom during emotional climax
            climax_t = (t - 17.0) / (duration_sec - 17.0)
            kb_scale = 1.08 + 0.10 * ease_out_cubic(climax_t)
        ...
        # -------------------------------------------------------------
        # 2. Left Torn Paper Panel: Slide-in (t: 0.05 -> end)
        # -------------------------------------------------------------
        if t >= 0.05:
        ...
        # -------------------------------------------------------------
        # 3. Chapter Badge ('CHƯƠNG 1'): Pop & Bounce (t: 0.35 -> end)
        # -------------------------------------------------------------
        if t >= 0.35:
        ...
        # -------------------------------------------------------------
        # 4. Headline Text & Yellow Brush Swipe (t: 0.60 -> end)
        # -------------------------------------------------------------
        if t >= 0.60:
        ...
        # -------------------------------------------------------------
        # 5. Callout Block 1: '2.7 TRIỆU NGƯỜI' (t: 1.40 -> end)
        # -------------------------------------------------------------
        if t >= 1.40:
            c1_t = min(1.0, (t - 1.40) / 0.60)
            c1_alpha = ease_out_cubic(c1_t)
            
            # Subtle dim during climax to reduce cognitive clutter
            if t >= 17.5:
                dim_prog = min(1.0, (t - 17.5) / 1.0)
                c1_alpha = 1.0 - 0.55 * dim_prog

            c1_offset_y = int(16.0 * (1.0 - ease_out_back(c1_t, 1.2)))
            c1_img = img_layers["callout1"]
            if c1_alpha < 0.99 or c1_offset_y > 0:
                arr = np.array(c1_img)
                arr[..., 3] = (arr[..., 3] * max(0.0, c1_alpha)).astype(np.uint8)
        ...
        # -------------------------------------------------------------
        # 6. Top-Right Cartography Map Card WITH EXIT ANIMATION (t: 2.75 -> 9.80s)
        # -------------------------------------------------------------
        if 2.75 <= t <= 9.80:
            ...
            if t < 8.80:
                ...
                # Graceful Exit: Fade out & float upward
                exit_t = min(1.0, (t - 8.80) / 0.85)
                map_alpha = 1.0 - ease_in_cubic(exit_t)
                total_map_y = int(-40.0 * ease_out_cubic(exit_t))

            arr = np.array(map_img)
            arr[..., 3] = (arr[..., 3] * max(0.0, map_alpha)).astype(np.uint8)
            f_temp = Image.fromarray(arr)

            m_shifted = Image.new("RGBA", (1920, 1080), (0, 0, 0, 0))
            m_shifted.paste(f_temp, (0, total_map_y))
        ...
        # -------------------------------------------------------------
        # 10. Continuous Dynamic Word-Highlight Subtitles (0.0s - 24.5s)
        # -------------------------------------------------------------
        draw_dynamic_subtitles_v3(frame, current_time=t, font=sub_font)
...
def encode_master_cut_v3_video(
    frames_dir: Path,
    audio_wav: Path,
    output_mp4: Path,
    fps: int = 30,
    ...
    print(f"🎬 Compiling Master Cut v3 storytelling video to {output_mp4}...")
    subprocess.run(cmd, capture_output=True, check=True)
    print("✓ Master Cut v3 video encoding complete!")


def main():
    work_dir = Path("/home/luuvandien/.gemini/antigravity-ide/brain/21dc8a17-be9c-4323-b4c2-5b4aa028e2a3/scratch/master_cut_v3")
    frames_dir = work_dir / "frames"
    hero_img = Path("artifacts/berlin_1961_archival.jpg")
    audio_wav = Path("artifacts/tts_cache/dbe89b0cb306b9a5.wav")
    out_mp4 = Path("artifacts/berlin_storytelling_master_v3.mp4")

    print("================================================================================")
    print("🚀 BẮT ĐẦU DỰNG PHIÊN BẢN MASTER CUT V3: EXIT LIFECYCLE & SEAMLESS SUBTITLES")
    print("================================================================================")

    print("Step 1: Tạo các lớp đồ họa Vector Cartography chính xác & Sign tinh chỉnh...")
    layers = build_cinematic_layers(work_dir)

    print("Step 2: Dựng hoạt ảnh chuyển động có Entrance/Exit & Subtitle liên tục 100%...")
    duration_sec = 24.5
    render_master_cut_v3_frames(hero_img, layers, frames_dir, duration_sec=duration_sec, fps=30)

    print("Step 3: Mux âm thanh Azure Neural Voice + Film Grain + Optical Vignette...")
    encode_master_cut_v3_video(frames_dir, audio_wav, out_mp4, fps=30)

    print("================================================================================")
    print(f"🎉 MASTER CUT V3 HOÀN TẤT! Video tại: {out_mp4}")
    print("================================================================================")
